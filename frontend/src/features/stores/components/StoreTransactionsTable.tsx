...
export function StoreTransactionsTable<T extends { id: number }>({
  columns,
  emptyDescription,
  emptyTitle,
  isLoading = false,
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {rows.map((row) => (
              <tr
              className={`hover:bg-brand-50 dark:hover:bg-slate-800/60 ${
                selectedRowId === row.id ? "bg-brand-100 dark:bg-slate-800" : ""
